...
def make_optimizer(
    ...
    if train_params.WEIGHT_DECAY > 0:
        if train_params.OPTIMIZER == 'adamw':
            if train_params.PRIOR_DECAY:
                no_decay = ['bias', 'LayerNorm.bias', 'LayerNorm.weight']
            # print(any([n == "prompt" for n, p in params]))
                ...
                optimizer_grouped_parameters = [
                    ...
                    {'params': [p for n, p in params
                                if any(nd in n for nd in no_decay)], 'weight_decay': 0.0}, 
                    {'params': [p for n, p in params
                                if n.endswith("prompt")], 'weight_decay': 0.0}          
                ]
            optimizer = AdamW(
                optimizer_grouped_parameters,
            ...
            for p in params:
                key, value = p
                # print(any(["prompt" in n for n, p in params]))
                # print(key)
                # if not value.requires_grad:
                #     continue
                lr = train_params.BASE_LR
                weight_decay = train_params.WEIGHT_DECAY
                if not train_params.PRIOR_DECAY and key.endswith("prompt"):
                    weight_decay = 0.0
