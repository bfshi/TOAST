...
class TFDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        im = to_torch_imgs(
            self._image_tensor_list[index], self.img_mean, self.img_std, self.cfg, input_range = (0.0, 1.0))
...
def preprocess_fn(data, size=224, input_range=(0.0, 1.0), cfg = None):
...
def to_torch_imgs(img: np.ndarray, mean: Tensor, std: Tensor, cfg, input_range) -> Tensor:
    if cfg.DATA.SEVERITY >= 1e-4:
        img = corrupt(img.astype(np.uint8), severity=cfg.DATA.SEVERITY, corruption_name = cfg.DATA.CORRUPTION)
    img = img.astype(np.float32) / 255.0
    img = img * (input_range[1] - input_range[0]) + input_range[0]
