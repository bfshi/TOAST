...
def setup(args, lr, wd, final_runs, run_idx=None, seed=None):
    ...
    cfg.DATA.NUMBER_CLASSES = DATA2CLS[cfg.DATA.NAME[5:] if cfg.DATA.NAME.startswith('vtab-') else cfg.DATA.NAME]
    ...
    if not cfg.SOLVER.PRIOR_DECAY:
        cfg.OUTPUT_DIR = cfg.OUTPUT_DIR + "_nopriordecay"

    if not final_runs:
        cfg.RUN_N_TIMES = 999999999
...
def train(cfg, args, final_runs):
    ...
    if train_loader:
        trainer.train_classifier(train_loader, val_loader, test_loader, save_best_test = final_runs)
        # save the evaluation results
        if not final_runs:
            print(">>>>>>>>>>>>>>>>>>>")
            for i in range(cfg.SOLVER.TOTAL_EPOCH):
                try:
                    print(evaluator.results[f"epoch_{i}"]["classification"][f"val_{cfg.DATA.NAME}"]["top1"])
                except KeyError:
                    break
            print(">>>>>>>>>>>>>>>>>>>")
        torch.save(
...
def get_lrwd_range(args):
    ...
    if args.grid_search_type == "finetune":
        lr_range = [0.001, 0.0001, 0.0005, 0.005]
        wd_range = [0.01, 0.001, 0.0001, 0.0]

    elif args.grid_search_type == "prompt":
        lr_range = [
            5.0, 2.5, 1.0,
            50.0, 25., 10.0,
            0.5, 0.25, 0.1, 0.05
    ...
    elif args.grid_search_type == "lora":
        lr_range = [
            1.0,
    ...
    elif args.grid_search_type == "topdown":
        lr_range = [
            1.0, 
            0.5, 0.25, 0.1, 0.05, 0.025, 0.01
        ]
